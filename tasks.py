...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SSOT_VSPHERE_xxx
namespace = Collection("nautobot_ssot_vsphere")
namespace.configure(
    {
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_ssot_vsphere.compose_http_timeout,
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_ssot_vsphere.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_ssot_vsphere"
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_ssot_vsphere'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="nautobot_ssot_vsphere", failfast=False, buffer=True):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_ssot_vsphere/*' --omit *migrations*"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.nautobot_ssot_vsphere.local):
